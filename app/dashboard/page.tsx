...
export default function Dashboard() {
  ...
  const totalBalance = wallets?.reduce((acc: number, w: any) => {
    return Math.round((acc + (Number(w.balance) || 0)) * 100) / 100;
  }, 0) || 0;
    ...
    <div className="min-h-screen bg-slate-50 pb-32">
      <header className="p-8 bg-white rounded-b-[3rem] shadow-sm">
        <div className="flex justify-between items-center mb-8">
          ...
          <Link href="/dashboard/profile" className="w-12 h-12 bg-indigo-50 rounded-2xl flex items-center justify-center text-xl border border-indigo-100">👤</Link>
        </div>
      ...
      <main className="p-6 space-y-8">
        <section>
          <div className="flex justify-between items-center mb-4 px-2">
            ...
            <Link href="/dashboard/wallet" className="text-indigo-600 text-[10px] font-black uppercase tracking-widest">+ Add wallet</Link>
          </div>
            ...
            {wallets?.length > 0 ? (
              wallets.map((wallet: any) => (
                <div key={wallet.id} className="min-w-[160px] bg-white p-5 rounded-3xl shadow-sm border-2 border-slate-50">
                  <div className={`w-8 h-8 rounded-lg mb-3 ${wallet.color || 'bg-indigo-500'}`} />
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-tight">{wallet.name}</p>
                  <p className="font-black text-slate-900">${Number(wallet.balance).toLocaleString('en-US', { minimumFractionDigits: 2 })}</p>
                </div>
              ))
            ) : (
              <div className="w-full py-8 text-center bg-white rounded-3xl border-2 border-slate-50">
                <p className="text-slate-400 font-bold text-xs uppercase tracking-widest">No wallets found</p>
              </div>
            )}
          </div>
        </section>
          ...
          <div className="space-y-3">
            {transactions?.length > 0 ? (
              transactions.slice(0, 8).map((tx: any) => (
                <div key={tx.id} className="bg-white p-4 rounded-[1.8rem] flex justify-between items-center border-2 border-slate-50">
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-lg ${tx.amount < 0 ? 'bg-rose-50 text-rose-500' : 'bg-emerald-50 text-emerald-500'}`}>
                      {tx.amount < 0 ? '↓' : '↑'}
                    </div>
                    <div>
                  ...
                  <div className="text-right flex items-center gap-3">
                    ...
                    <button onClick={() => deleteTransaction(tx.id)} className="p-2 text-slate-200 hover:text-rose-500 transition-colors">✕</button>
                  </div>
                </div>
              ))
            ) : (
      ...
      <nav className="fixed bottom-6 left-6 right-6 bg-white/90 backdrop-blur-md border border-slate-200 flex justify-around items-center p-3 rounded-[2rem] shadow-2xl z-50">
        ...
        <Link href="/dashboard/add" className="bg-indigo-600 w-14 h-14 rounded-2xl flex items-center justify-center text-white text-2xl shadow-lg transform active:scale-95 transition-all">+</Link>
